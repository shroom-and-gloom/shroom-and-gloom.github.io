...
import type { InternalLink, PageSection, PageSource, SeoPageDefinition } from "../config/types";
import { homePage } from "../content/home";
import { allPages, enabledCorePages, enabledPages } from "../content/registry";

const allowedFactsStatus = new Set(["skeleton", "verified", "mixed", "player-reported"]);
const allowedSourceTypes = new Set(["official", "steam-data", "player-reported"]);
...
function validatePage(page: SeoPageDefinition) {
  ...
  if (page.factsStatus && !allowedFactsStatus.has(page.factsStatus)) {
    fail(`${page.slug}: invalid factsStatus ${page.factsStatus}`);
  }
  validateSources(page.slug, page.sources ?? []);
}

function validateSources(owner: string, sources: PageSource[]) {
  for (const source of sources) {
    if (!source.label?.trim() || !source.typeLabel?.trim() || !source.reviewed?.trim()) {
      fail(`${owner}: source entries need label, typeLabel, and reviewed date`);
    }
    if (!allowedSourceTypes.has(source.type)) fail(`${owner}: invalid source type ${source.type}`);
    if (!isoDatePattern.test(source.reviewed) || Number.isNaN(Date.parse(source.reviewed))) {
      fail(`${owner}: invalid source reviewed date for ${source.label}`);
    }
    try {
      const host = new URL(source.url).hostname;
      if (host !== siteHost && !siteConfig.allowedExternalDomains.includes(host)) {
        fail(`${owner}: source domain is not allowlisted: ${host}`);
      }
    } catch {
      fail(`${owner}: invalid source URL for ${source.label}`);
    }
  }
}
...
if (homePage.factsStatus && !allowedFactsStatus.has(homePage.factsStatus)) {
  fail(`home: invalid factsStatus ${homePage.factsStatus}`);
}
validateSources("home", homePage.sources ?? []);
