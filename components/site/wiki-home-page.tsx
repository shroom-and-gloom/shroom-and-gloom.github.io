...
export function WikiHomePage() {
  ...
  const tocItems = wikiTocItems(homePage.sections, [
    ...
    ...(homePage.sources?.length ? [{ id: "sources-verification", heading: "Sources & verification" }] : []),
            ...
            sidebar={(
              <>
                ...
                <section className="wiki-site-info" aria-label="Site info">
                  ...
                  <img src={assetPath(siteConfig.assets.cover)} alt={`${siteConfig.game.name} cover art`} width={640} height={400} />
                  <dl>
                    {facts.map(([term, value]) => (
                      <div key={term}>
                        <dt>{term}</dt>
