...
function WikiSectionBlock({ section }: { section: PageSection }) {
  return (
...
export function WikiPageSections({
  sections,
  afterFirstSection,
}: {
  ...
  if (!afterFirstSection || sections.length === 0) {
    return (
  ...
  return (
    <div className="wiki-page-sections">
      <WikiSectionBlock section={first} />
      {afterFirstSection}
      {rest.map((section) => (
        <WikiSectionBlock key={section.id} section={section} />
      ))}
